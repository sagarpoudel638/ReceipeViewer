...
function Typecard({ type }) {
  ...
  const [selectedFoodLabel, setSelectedFoodLabel] = useState(""); // State for selected food label
  ...
  const getFoods = () => {
    if (type === "Global Cuisine") {
      ...
      return [
        { foodLabel: "Grilled Chicken With Vegetables", key: "grilledchicken" },
      ];
    } else {
  ...
  const handleFoodClick = (foodLabel) => {
    setSelectedFoodLabel(foodLabel); 
  ...
  return (
    <>
        ...
        <div className="p-5">
          ...
          <div className="w-58 text-lg font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            {foods.map((food) => (
              <button
                key={food.key}
                onClick={() => handleFoodClick(food.foodLabel)}
      ...
      {/* Modal */}
      {isModalVisible && (
        <ReceipeViewer setIsModalVisible= {setIsModalVisible} 
        foodLabel={selectedFoodLabel} 
        />
      )}
    </>
  );
